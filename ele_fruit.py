...
class LYB:
    ...
    def xsign(self, api, data, wua, v):
        url = "http://154.83.12.228:9922/api/getXSign"
        body = {
            "data": data,
            "api": api,
            "pageId": '',
...
def tq2(title: str, content: str) -> None:
    ...
    timestamp = str(round(time.time() * 1000));secret_enc = test.get("test1").encode("utf-8")
    string_to_sign = "{}\n{}".format(timestamp, test.get("test1"))
    string_to_sign_enc = string_to_sign.encode("utf-8");hmac_code = hmac.new(
        secret_enc, string_to_sign_enc, digestmod=hashlib.sha256
    ).digest();sign = urllib.parse.quote_plus(base64.b64encode(hmac_code))
